...
| 客户新增 | 输入名称并查重 | 候选或无匹配后进入表单 | 强重复阻止新增；近似候选需填写理由 |
| 沟通录入 | 手工表单或 AI 材料 | 手工可直接正式保存；AI 逐步到人工确认 | 缺项就地提示；AI 失败可重试或转手工 |
| 停用治理 | 查看停用对象 | 默认隐藏，治理视图可查看原因并恢复 | 历史只读，不级联删除 |
...
目录覆盖工作台、客户主流程、沟通主流程、AI 材料到人工确认、补充差异、停用治理和我的。瞬时状态在所属页面内交互切换，避免复制画板。
...
- 在桌面与三种手机视口完成客户、手工沟通、AI 草稿确认、停用恢复和意见保存/导出路径。

**Browser regression:**
- 手工正式记录必填、草稿保留、强重复阻断和客户名称上下文传递通过。
- AI 空材料拦截、候选选择、补充已有记录、字段编辑保留与版本冲突阻断通过。
- 销售角色治理拒绝、停用原因必填、逐页意见持久化和 375 / 430 像素视口通过。
